  ...
  def apply(dm: DatasetMetadata): Try[Elem] = Try {
    ...
    >
      <ddm:profile>
        ...
        { dm.datesCreated.map(_.value).withNonEmpty.map(str => <ddm:created>{ str }</ddm:created>) }
        { dm.datesAvailable.map(_.value).withNonEmpty.map(str => <ddm:available>{ str }</ddm:available>) }
