...
class DatasetXmlSpec extends TestSupportFixture {
  ...
  it should "produce all types of dates" in {
    ...
    val dates = DateQualifier.values.toSeq.map { q =>
      QualifiedSchemedValue[String, DateQualifier](Some("dcterms:W3CDTF"), date, q)
    }
    // need namespace because DatasetXml.elemLabel returns plain strings
    val ddm = toDDM(minimal.copy(dates = Some(dates.toSeq)))
    ...
    (ddm \ "dcmiMetadata" \ "dcterms:modified").text shouldBe date
    ...
    // the dateSubmitted specified by this test and one generated by DatasetXml.apply in toDDM:
    (ddm \ "dcmiMetadata" \ "dcterms:dateSubmitted").text shouldBe date + "2018-03-22"
  }
